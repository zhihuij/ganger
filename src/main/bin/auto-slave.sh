...
#if slave home is not set or is not a directory
...
MASTER_OPTS="${MASTER_OPTS} -DslaveHome=${MASTER_HOME}"
...
master_exec_command="exec \"$JAVACMD\" -server $JVM_SETTINGS $MASTER_OPTS -classpath \"$LOCALCLASSPATH\" com.netease.automate.slave.AutoSlave $*"
echo $master_exec_command
